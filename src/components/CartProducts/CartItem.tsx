...
import {  CartItem, addToCart, removeFromCart  } from "../../services/products/cartSlice";
...
export const  CartItems =  ({ product }: Props) =>{
    ...
    return (
        <Card key={product.id + product.rating.count + product.rating.rate} sx={{
            ...
            <CardContent sx={{display: "flex", flexDirection:'column', maxHeight: 40}}>
             <Typography variant='h6'>{product.title}</Typography>
            </CardContent>
            <CardContent sx={{display: "flex", flexDirection: "row", justifyContent:'space-between'}}>
                    <p>Price: ${product.price}</p>
            <CardMedia component="img"  sx={{ objectFit:"scale-down", alignItems:'left', width: 90}} image={product.image} />
            </CardContent>
